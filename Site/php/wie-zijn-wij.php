<!DOCTYPE html>
<html lang="en">
<head>
    <title>VrijheidVoorIeder.</title>
    <link rel="stylesheet" href="../css/style-wie-zijn-wij.css">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Montserrat&display=swap" rel="stylesheet">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Roboto&display=swap" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@700&display=swap" rel="stylesheet">
</head>
<body>
<header>
        <div class="grid">
            <div class="header-left">
                <a class="header" href="../php/wie-zijn-wij.php">Wie zijn wij?</a>
                <a class="header" href="../php/onzevisie.php">Onze visie.</a>
                <a class="header" href="../php/agenda.php">Agenda.</a>
            </div>
            <div class="logo">
                <a class="header" href="../php/index.php"><img src="../../logo/logo_small_icon_only.png"></a>
            </div>
            <div class="header-right">
                <a class="header" href="../php/login/login.php">Log In/Register.</a>
                <a class="header" href="../php/contact.php">Contact.</a>
            </div>
        </div>
    </header>
    <main>
        <div class="titel">
            <h1>Onze politici!</h1>
        </div>
        <div class="poli">
            <div class="noach">
                <H3>Noach Maurits</H3>
                <img src="../../faces/istockphoto-517998264-612x612.jpg">
                <p>Wil je meer over Noach weten? Klik dan op de knop hieronder!</p>
                <a href="#"><button>Noach</button></a>
            </div>
            <div class="frans">
                <h3>Frans Manfred</h3>
                <img src="../../faces/istockphoto-517998264-612x612.jpg">
                <p>Wil je meer over Frans weten? Klik dan op de knop hieronder!</p>
                <a href="#"><button>Frans</button></a>
            </div>
            <div class="ade">
                <h3>Ade Ekwueme Kwadwo</h3>
                <img src="../../faces/istockphoto-517998264-612x612.jpg">
                <p>Wil je meer over Ade weten? Klik dan op de knop hieronder!</p>
                <a href="#"><button>Ade</button></a>
            </div>
            <div class="yefrem">
                <h3>Yefrem Georgry</h3>
                <img src="../../faces/istockphoto-517998264-612x612.jpg">
                <p>Wil je meer over Yefrem weten? Klik dan op de knop hieronder!</p>
                <a href="#"><button>Yefrem</button></a>
            </div>
            <div class="jin">
                <h3>Jin Yu</h3>
                <img src="../../faces/istockphoto-517998264-612x612.jpg">
                <p>Wil je meer over Jin weten? Klik dan op de knop hieronder!</p>
                <a href="#"><button>Jin</button></a>
            </div>
            <div class="seo">
                <h3>Seo-Yeon Min-Su</h3>
                <img src="../../faces/istockphoto-517998264-612x612.jpg">
                <p>Wil je meer over Seo-Yeon weten? Klik dan op de knop hieronder!</p>
                <a href="#"><button>Seo-Yeon</button></a>
            </div>
            <div></div>
            <div class="aisha">
                <h3>A'isha Mouna</h3>
                <img src="../../faces/istockphoto-517998264-612x612.jpg">
                <p>Wil je meer over A'isha weten? Klik dan op de knop hieronder!</p>
                <a href="../../partijleden/a'isha.php"><button>A'isha</button></a>
            </div>
        </div>
    </main>
    <footer>
        <div class="contact">
            <h2>Heb je vragen over iets? Of wil je wat laten weten? Klik dan op de knop hier onder om met ons in contact te komen!</h2>
            <a href="../php/contact.php"><button class="button2">Klik hier!</button></a>
    </footer>
</body>
</html>